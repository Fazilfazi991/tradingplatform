...
def main() -> None:
    ...
    manifest = build_manifest(
        purpose="Batch 1 golden fixture verification",
        universe={"code": "GOLDEN_FIXTURE", "version": "1", "point_in_time": True},
        bars=bars,
        file_references=[{"path": target.as_posix(), "sha256": file_hash}],
        code_sha=code_sha,
        trading_calendar_version="GOLDEN_1",
        corporate_action_version="UNRESOLVED_FIXTURE",
        ...
        json.dumps(
            {
                ...
                "date_start": min(bar.session_date for bar in bars),
                "date_end": max(bar.session_date for bar in bars),
                "quality_events": len(events),
                "quarantined": len(quarantine),
                "parquet_sha256": file_hash,
                "dataset_sha256": sealed["sha256"],
