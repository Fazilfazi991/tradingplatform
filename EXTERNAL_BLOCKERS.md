...
| EXT-06 | Hosting and domain | Owner-approved canonical domain, protected staging/production release environments with scoped credentials, successful staged build, production promotion, smoke and rollback evidence | A Vercel project is linked and its root defect is resolved, but no successful or approved deployment evidence exists | Keep automatic Git deployment disabled; use only the fail-closed manual release workflow after approvals | OPEN |
